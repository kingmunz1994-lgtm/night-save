/**
 * deploy.ts — Deploy NightSave to Midnight Preprod
 *
 * Run:
...
const ZK_CONFIG_PATH = path.resolve(import.meta.dirname, '..', 'contracts', 'managed', 'night-save');
...
async function main() {
  ...
  console.log('\n🚀 Deploying NightSave to Midnight Preprod...\n');
  ...
  const compiled = CompiledContract
    .make('night-save', ContractModule.Contract)
  ...
  const providers = {
    ...
    privateStateProvider: levelPrivateStateProvider({ privateStateStoreName: 'night-save-deploy', walletProvider: ctx.wallet as any }),
  ...
  const deployed = await deployContract(providers, { compiledContract: compiled, privateStateId: 'nightSaveState', initialPrivateState: {} });
